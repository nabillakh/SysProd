		...
		<div id="list-test" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="nom" title="${message(code: 'test.nom.label', default: 'Nom')}" />
					
						<g:sortableColumn property="test" title="${message(code: 'test.test.label', default: 'Test')}" />
				...
				<g:each in="${testInstanceList}" status="i" var="testInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${testInstance.id}">${fieldValue(bean: testInstance, field: "nom")}</g:link></td>
					
						<td><g:formatBoolean boolean="${testInstance.test}" /></td>
